...
class Model(BaseModel):
    def __init__(self,
        ...
        self.sb_bgm_model = SequenceModel(
            input_size=fb_num_neighbors * 2 + 1,
            output_size=1,
    ...
    def forward(self, noisy_mag, bgm_mag):
        ...
        #=======================================================
        #new implementation: TODO validate that everything works correctly

        bgm_mag_unfolded = self.unfold(bgm_mag, num_neighbor=self.fb_num_neighbors)
        bgm_mag_unfolded = bgm_mag_unfolded.reshape(batch_size, num_freqs, self.fb_num_neighbors * 2 + 1, num_frames) #[B, F, F_f, T]
        # would be possible to concat noisy rep here.
        sb_bgm_input = self.norm(bgm_mag_unfolded)

        # if batch_size > 1:
        #     sb_bgm_input = sb_bgm_input.permute(0, 2, 1, 3) # [B, (F_f), F, T] #DROPPING BANDS HERE (AND DROPPING NOISY BANDS BEFORE CONCAT) SHOULD IMPROVE PERFORMANCE
        #     num_freqs = sb_bgm_input.shape[2]
        #     sb_bgm_input = sb_bgm_input.permute(0, 2, 1, 3)  # [B, F, (F_f), T]
        # above code actually doing nothing right now

        sb_bgm_input = sb_bgm_input.reshape( #[B*F, F_f, T]
            batch_size * num_freqs,
            self.fb_num_neighbors * 2 + 1,
            num_frames
        )

        sb_bgm_output = self.sb_bgm_model(sb_bgm_input)
        sb_bgm_output = sb_bgm_output.reshape(batch_size, num_freqs, 1, num_frames)
        sb_bgm_output = sb_bgm_output.permute(0, 2, 1, 3) # should now be [B, 1, F, T]
        # end new implementation
        #=======================================================
        
        # Unfold the output of the fullband model, [B, N=F, C, F_f, T]
        sb_bgm_output_unfolded = self.unfold(sb_bgm_output, num_neighbor=self.fb_num_neighbors)
        sb_bgm_output_unfolded = sb_bgm_output_unfolded.reshape(batch_size, num_freqs, self.fb_num_neighbors * 2 + 1, num_frames)
        ...
        sb_input = torch.cat([noisy_mag_unfolded, sb_bgm_output_unfolded], dim=2)
