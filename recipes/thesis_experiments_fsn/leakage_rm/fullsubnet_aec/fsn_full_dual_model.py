...
class Model(BaseModel):
    def __init__(self,
        ...
        self.sb_model = SequenceModel(
            input_size= (sb_num_neighbors * 2 + 1) + (sb_num_neighbors * 2 + 1) + (fb_num_neighbors * 2 + 1),
            output_size=2,
    ...
    def forward(self, noisy_mag, bgm_mag):
        ...
        fb_output_unfolded = self.norm(fb_output_unfolded)
        ...
        # Concatenation, [B, F, (F_s + F_s + F_f), T]
        sb_input = torch.cat([noisy_mag_unfolded, bgm_mag_unfolded, fb_output_unfolded], dim=2)
        ...
        if batch_size > 1:
            sb_input = drop_band(sb_input.permute(0, 2, 1, 3), num_groups=self.num_groups_in_drop_band)  # [B, (F_s + F_s + F_f), F//num_groups, T]
            num_freqs = sb_input.shape[2]
            sb_input = sb_input.permute(0, 2, 1, 3)  # [B, F//num_groups, (F_s + F_s + F_f), T]

        sb_input = sb_input.reshape(
            batch_size * num_freqs,
            (self.sb_num_neighbors * 2 + 1) + (self.sb_num_neighbors * 2 + 1) + (self.fb_num_neighbors * 2 + 1),
            num_frames
        )

        # [B * F, (F_s + F_s + F_f), T] => [B * F, 2, T] => [B, F, 2, T]
