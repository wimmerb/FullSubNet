...
class DTLN(nn.Module):
    ...
    def __init__(self,
        ...
        self.layer_norm_12 = nn.LayerNorm(window_size // 2 + 1, eps=self.eps) #aec
        self.lstm_11 = nn.LSTM((self.window_size // 2 + 1) * 2, self.hidden_size) #aec -> * 2
        ...
        self.conv1d_21 = nn.Conv1d(self.window_size,
                                   ...
                                   stride=1,
                                   bias=False)
        self.conv1d_22 = nn.Conv1d(self.window_size, #aec
                                   self.encoder_size,
                                   kernel_size=1,
                                   stride=1,
                                   bias=False)
        self.layer_norm_21 = nn.LayerNorm(self.encoder_size, eps=self.eps)
        self.layer_norm_22 = nn.LayerNorm(self.encoder_size, eps=self.eps) #aec
        self.lstm_21 = nn.LSTM(self.encoder_size * 2, self.hidden_size) #aec -> * 2
    ...
    def init_weights_and_biases(self):
        ...
        self._init_layer_norm(self.layer_norm_12)
        self._init_layer_norm(self.layer_norm_21)
        self._init_layer_norm(self.layer_norm_22)
        ...
        torch.nn.init.xavier_uniform_(self.conv1d_22.weight)
    ...
    def forward(self, x, bgm):
        ...
        Args:
            x (torch.tensor): Input tensor corresponding to a single audio file
                or a batch.
        """ 
        batch_size_special = x.shape [0]

        _chunk_samples = x.shape[1]
        ...
        x_mag = torch.log10(x_mag_**2 + self.eps) #AEC using log - pow

        # normalize log stft
        x_mag = self.layer_norm_11(x_mag)

        # (batch, frames, bins) -> (frames, batch, bins)
        # e.g. (1, 1872, 257) -> (1872, 1, 257)
        ...
        # (batch, samples) -> (batch, bins, frames)
        # e.g. (1, 240000) -> (1, 257, 1872), (1, 257, 1872) 
        bgm_mag, bgm_phase = self.stft(bgm) 

        # (batch, bins, frames) -> (batch, frames, bins)
        # e.g. (1, 257, 1872) -> (1, 1872, 257)
        # TODO: Probably unnecessary!
        # NOTE: This is used to retrieve segments for 1d-conv later. Similar shape to First core output
        bgm_mag_ = bgm_mag.permute(0, 2, 1)
        # e.g. (1, 257, 1872) -> (1, 1872, 257)
        bgm_phase_ = bgm_phase.permute(0,2,1)

        # log norm (original uses Napierian log without scalar multiplier 10)
        bgm_mag = torch.log10(bgm_mag_**2 + self.eps) #AEC using log_pow

        # normalize log stft
        bgm_mag = self.layer_norm_12(bgm_mag)

        # (batch, frames, bins) -> (frames, batch, bins)
        # e.g. (1, 1872, 257) -> (1872, 1, 257)
        # lstm will take (seq, batch, features) as input
        bgm_mag = bgm_mag.permute(1, 0, 2)


        ######## START OF FIRST SEPARATION CORE ########

        x = torch.cat([x_mag, bgm_mag], dim=2)   ###AEC

        # (frames, batch, bins) -> (frames, batch, features)
        # e.g. (1872, 1, 257) -> (1872, 1, 128)
        x, (h0_12, c0_12) = self.lstm_11(x)
        ...
        # (batch, features, frames) -> (batch, frames, samples)
        # e.g. (1, 1872, 257) -> (1, 1872, 512)
        bgm = self.ifft(bgm_mag_, bgm_phase_)

        # accommodate input for conv1d
        # (batch, frames, samples) -> (batch, samples, frames)
        # e.g.(1, 1872, 512) -> (1, 512, 1872)
        bgm = bgm.permute(0, 2, 1)

        # (batch, samples, frames) -> (batch, features, frames)
        # e.g. (1, 512, 1872) -> (1, 256, 1872)
        bgm = self.conv1d_22(bgm)
        ...
        # (batch, frames, features) -> (batch, frames, features)
        # e.g. (1, 1872, 256) -> (1, 1872, 256)
        bgm = self.layer_norm_22(bgm)
        


        ######## START OF SECOND SEPARATION CORE ########
        x = torch.cat([x, bgm], dim=2)   ###AEC
        ...
        # AEC make this flexible in terms of size...
        overlap_and_add = nn.Fold((1, _chunk_samples),
                                    kernel_size=(1, self.window_size),
                                    stride=(1, self.hop_size))
        x = overlap_and_add(x)
