...
class Dataset(BaseDataset):
    def __init__(self,
        ...
        assert 0 <= pass_proportion <= 1, "reverberation proportion should be in [0, 1]"
        ...
        print ("pass_prop", self.pass_proportion)
    ...
    def _select_noise_y(self, target_length):
        ...
        pitch_shift = 0
        if bool(np.random.random(1) < 0.3):
            pitch_shift = np.random.randint(-1200, 1200) # octave
    ...
    @staticmethod
    def snr_mix(clean_y, noise_y, conv_y, snr, target_dB_FS, target_dB_FS_floating_value, use_reverb=True, eps=1e-6, num_trainfile_examples=10, sr=16000, do_pass=False):
        """
        ...
        Returns:
            (noisy_y，clean_y)
        """
        if bool(np.random.random(1) < 0.01):
        ...
        if num_trainfile_examples > 0:
            ...
            save_wav(f"{num_trainfile_examples}{'_rev' if use_reverb else ''}_noisy.wav", noisy_y, sr)
            save_wav(f"{num_trainfile_examples}{'_rev' if use_reverb else ''}_clean.wav", clean_y, sr)

        return noisy_y, clean_y

    
    def _get_sample(self, path, resample=None, pitch_shift = 0):
        if pitch_shift == 0:
            effects = [
                ["remix", "1"]
            ]
    ...
    def _get_sample_norm(self, snd_path, resample=None, processed=False, pitch_shift = 0):
        # print ("RIR SAMPLE")
        snd_raw, sample_rate = self._get_sample(snd_path, resample=resample, pitch_shift = pitch_shift)
        # print (rir_raw.shape)
        if not processed:
    ...
    def __getitem__(self, item):
        ...
        do_pass = bool(np.random.random(1) < self.pass_proportion)
